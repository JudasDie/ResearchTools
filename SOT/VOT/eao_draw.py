...
parser.add_argument('--dataset', default='VOT2017', type=str, help='Year of VOT benchmark')
...
def draw_eao(args, max_nums=30, legend_cols=7, legend_rows=3):
    fig, ax = plt.subplots(figsize=(8, 3))   # set width and height
    ...
    plt.legend(flip(handles, 7), flip(labels, 7), loc=9, ncol=legend_cols, bbox_to_anchor=(0.5, -0.3),
    ...
    ax.tick_params(width=3, direction='in', labelsize=18, pad=8)
    ...
    plt.ylabel('EAO', fontsize=20)
    plt.title('EAO Scores Rank on {}'.format(args.dataset), fontsize=20)
    plt.grid(axis='x', linewidth=3, ls='-', c='gainsboro')
    plt.grid(axis='y', linewidth=3, ls='-', c='gainsboro')
    ax.grid(alpha=1)


    save_name1 = 'eao_{}.png'.format(args.dataset)
    save_name2 = 'eao_{}.pdf'.format(args.dataset)
