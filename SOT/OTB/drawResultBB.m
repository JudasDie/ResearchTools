...
%     if ~strcmp(seq_name, 'motorRolling')
%         continue
%     end
...
%         text(10, 15, ['#' id], 'Color','y', 'FontWeight','bold', 'FontSize',24);
        ...
        save_frame = frame2im(getframe(gcf));
        save_frame = imresize(save_frame, [480 640]);
        imshow(save_frame)
        text(10, 15, ['#' id], 'Color','y', 'FontWeight','bold', 'FontSize',24);
        
        save_frame2 = frame2im(getframe(gcf));
        imwrite(save_frame2, [pathSave  num2str(i) '.png']);
    end
    clf
end
